...
export default function YouTubeDubber() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      try {
        response = await fetch(`${API_BASE}/api/dub-video`, {
          method: 'POST',
          headers: {
        ...
        throw new Error(`Không kết nối được backend tại ${API_BASE}. Hãy bật DubFlow backend và thử lại.`);
      }
      ...
      reportError(err.message || 'Something went wrong. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
